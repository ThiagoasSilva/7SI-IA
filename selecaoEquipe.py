...
#   a) Ordenar/filtrar estados na BFS
#   b) Compor a função fitness do AG
...
def bfs():
    ...
    while fila:
        ...
        # == PODA: custo já excede orçamento, não adianta continuar ==========
        if custo_total(tuple(estado_parcial)) > ORCAMENTO:
            continue

        # == PODA HEURÍSTICA: se nem somando todos os restantes cobrimos as ==
        # habilidades, descartamos (poda por inviabilidade futura)
        ...
        if not HABILIDADES_NECESSARIAS.issubset(skills_possiveis):
            continue  # impossível cobrir as habilidades restantes
...
#
# == 4.2 Função Fitness ======================================================
# Queremos MAXIMIZAR: performance total (bônus por cobrir habilidades)
# Penalizamos: extrapolação do orçamento e habilidades faltando
#
# fitness = performance_total
#           + 10 * habilidades_cobertas_do_necessário
...
# =============================================================================
# FUNÇÕES AUXILIARES — substituem os lambdas originais
# =============================================================================
#
# Antes, o código usava lambda em dois lugares para "extrair o fitness de um
# índice". Aqui transformamos cada uso em uma função com nome descritivo,
# tornando o código mais fácil de ler e depurar.

def pegar_fitness_do_candidato(indice_candidato, fitnesses):
    """
    Retorna o valor de fitness de um candidato pelo seu índice na lista.

    Por que existe?
    ---------------
    Usada como critério de comparação no torneio de seleção (selecao_torneio).
    Permite que max() saiba QUAL candidato tem o maior fitness, sem precisar
    de um lambda anônimo.

    Exemplo:
        fitnesses = [10, 30, 20]
        pegar_fitness_do_candidato(1, fitnesses)  ->  30
    """
    return fitnesses[indice_candidato]


def pegar_fitness_do_individuo(indice_individuo, fitnesses):
    """
    Retorna o valor de fitness de um indivíduo pelo seu índice na população.

    Por que existe?
    ---------------
    Usada como critério de ordenação dentro do algoritmo genético (sorted).
    Permite ordenar os índices da população do maior para o menor fitness,
    sem precisar de um lambda anônimo.

    Exemplo:
        fitnesses = [5, 15, 10]
        pegar_fitness_do_individuo(1, fitnesses)  ->  15
    """
    return fitnesses[indice_individuo]


# =============================================================================
# NOTA: as duas funções acima fazem a mesma coisa (acessar fitnesses[i]).
# Elas foram separadas em duas para deixar claro ONDE cada uma é usada:
#   • pegar_fitness_do_candidato -> usada em selecao_torneio
#   • pegar_fitness_do_individuo -> usada em algoritmo_genetico
# Se preferir, você pode unificá-las em uma só função chamada, por exemplo,
# "pegar_fitness(i, fitnesses)".
# =============================================================================
...
def selecao_torneio(populacao, fitnesses):
    ...
    Agora (com função nomeada):
        usamos pegar_fitness_do_candidato via key= com wrapper abaixo.
    """
    candidatos = random.sample(range(len(populacao)), TAM_TORNEIO)

    # Precisamos passar fitnesses para a função auxiliar.
    # Como key= só aceita função de 1 argumento, criamos uma função interna
    # que "captura" fitnesses do escopo local — mesmo comportamento do lambda.
    def criterio_torneio(indice_candidato):
        return pegar_fitness_do_candidato(indice_candidato, fitnesses)
...
def algoritmo_genetico():
    ...
    for geracao in range(1, NUM_GERACOES + 1):
        ...
        #
        # Agora (com função nomeada):
        #   usamos pegar_fitness_do_individuo via wrapper abaixo.
        def criterio_ordenacao(indice_individuo):
            return pegar_fitness_do_individuo(indice_individuo, fitnesses)

        indices_ordenados = sorted(
            range(TAM_POPULACAO),
            key=criterio_ordenacao,
            reverse=True   # do maior fitness para o menor
        )
...
if __name__ == "__main__":
    ...
    # Exemplo: selecionar apenas Alice
